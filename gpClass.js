...
class GamePlan{
    ...
    currentLevels(type = ['time', 'task', 'milestone'], setTime){
        ...
        if(type.includes('time')){
            ...
            let currentTime = this.#currentLevels(currentDate);
            returnObj.time = {};
        ...
        if(type.includes('task')){
            ...
            let currentTask = this.#currentLevels(currentDate);
            returnObj.task = {};
    ...
    updateTime(type = ['time', 'task', 'milestone']){
        type.forEach(el=>{
            window.dispatchEvent(new CustomEvent(`gp-update-${el}`, {detail: this.currentLevels(type)[el]}));
